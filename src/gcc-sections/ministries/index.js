        ...
        <Grid item spacing={3} container>
          {ministries.map((ministry, index) => (
            <Grid key={index} item xs={12} sm={6} md={5} lg={4} container justifyContent='center'>
              <MinistryCard handleOpen={handleOpen} ministry={ministry} />
            </Grid>
          ))}
        </Grid>
      </Grid>
...
function MinistryCard({ ministry, handleOpen }) {
  ...
  return (
    <Box sx={{ borderRadius: 2, width: 1, overflow: 'hidden', position: 'relative' }}>
      ...
      >
        ...
        <Button size='small' sx={{ alignSelf: 'flex-start', px: 0}} onClick={() => handleOpen(ministry)}>
          Read More
        </Button>
      </Stack>
